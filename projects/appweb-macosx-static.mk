...
all compile: prep \
        ...
        $(CONFIG)/bin/libappweb.a \
        $(CONFIG)/bin/libmod_esp.a \
        $(CONFIG)/bin/esp \
        $(CONFIG)/bin/esp.conf \
        src/server/esp.conf \
        $(CONFIG)/bin/esp-www \
        ...
        $(CONFIG)/bin/ejsc \
        $(CONFIG)/bin/ejs.mod \
        $(CONFIG)/bin/libmod_cgi.a \
        $(CONFIG)/bin/libmod_ssl.a \
        $(CONFIG)/bin/authpass \
...
clean:
	...
	rm -rf $(CONFIG)/bin/libappweb.a
	rm -rf $(CONFIG)/bin/libmod_esp.a
	...
	rm -rf $(CONFIG)/bin/libmod_cgi.a
	rm -rf $(CONFIG)/bin/libmod_ssl.a
	rm -rf $(CONFIG)/bin/authpass
	rm -rf $(CONFIG)/bin/cgiProgram
	rm -rf $(CONFIG)/bin/setConfig
	rm -rf $(CONFIG)/bin/libapp.a
...
$(CONFIG)/bin/ejs:  \
        $(CONFIG)/bin/libejs.a \
        $(CONFIG)/obj/ejs.o
	$(CC) -o $(CONFIG)/bin/ejs -arch x86_64 $(LDFLAGS) $(LIBPATHS) $(CONFIG)/obj/ejs.o -lejs $(LIBS) -lmpr -lpcre -lhttp -lpam -ledit -ledit
...
install-prep:  \
        compile
	cd . >/dev/null ;\
		$(eval $(shell $(BIN)/ejs bits/getbitvals projects/$(NAME)-$(OS)-$(PROFILE)-bit.h PRODUCT VERSION CFG_PREFIX PRD_PREFIX WEB_PREFIX LOG_PREFIX BIN_PREFIX SPL_PREFIX BIN_PREFIX  ;\
